    ...
    public class User : EntityBase<PridContext>
    {
        ...
        public IQueryable<Tricount> GetFiltered(string Filter) {
            var filtered = from t in GetTricounts().Union(GetParticipatedTricounts())
                ...
                where t.Title.ToLower().Contains(Filter) || t.Creator.FullName.ToLower().Contains(Filter) ||
        ...
        public IQueryable<Tricount> GetTricountsWithNoParticipation() {
            // Récupère les ID des tricounts auxquels l'utilisateur participe
            var participatedTricountIds = Context.Subscriptions
                .Where(s => s.UserId == UserId)
                .Select(s => s.TricountId);

            // Sélectionne tous les tricounts excepté ceux auxquels l'utilisateur participe
            var tricounts = Context.Tricounts
                .Where(t => !participatedTricountIds.Contains(t.Id))
                .OrderBy(t => t.Title);
